...
def main():
    ...
    if auth_token:
        ...
        response = upsert_datatype(write_host, auth_token, prn, prv, "usage", "times")
    ...
    for name in names:
        ...
        usage = get_used_charging(read_host,name)
        ...
        response = add_station_data(write_host, auth_token, provenance_id, station_type, name, round(time.time() * 1000), "usage", usage)
        print(f"Station {name} available: {availability}%.")
        print(f"Station {name} used: {usage} times.")
